function Objects(main) {
    ...
    this.stateChange = function (id, state) {
        if (this.$grid)
            this.$grid.selectId('state', id, state);
    };

    this.objectChange = function (id, obj) {
        if (this.$grid)
            this.$grid.selectId('object', id, obj);
    };

    this.reinit = function () {
        this.checkCustoms();
        if (this.$grid)
            this.$grid.selectId('reinit');
    };

    this.resize = function (width, height) {
        if (this.$grid)
            this.$grid.height(height - 100).width(width - 20);
    };
                ...
                buttons: [
                    {
                        ...
                        match: function (id) {
                            if (!that.main.objects[id])
                                this[0].outerHTML = '<div style="width: 26px; height: 1px; display: inline-block;"></div>';
                        },
                        width: 26,
                        height: 20
                    },
                ...
                settings.customButtonFilter = {
                    ...
                    callback: function () {
                        ...
                        for (var i = 0; i < _ids.length; i++) {
                            if (that.main.objects[_ids[i]] && that.main.objects[_ids[i]].type === 'state')
                                ids.push(_ids[i]);
                        }
                        if (ids && ids.length) {
    ...
    this.edit = function (id, callback) {
        ...
        if (this.main.config['object-edit-width'])
            width = this.main.config['object-edit-width'];
        if (this.main.config['object-edit-height'])
            height = this.main.config['object-edit-height'];
